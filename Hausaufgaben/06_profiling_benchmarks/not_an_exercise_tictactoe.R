...
ex02MoveStringToCoordinate <- function(column, position) {
  ...
  row <- max(c(which(is.na(position[, col])), 0))  # c(..., 0) to avoid warning when which() gives length-0 result
  if (row == 0) {
...
ex03HumanPlayer <- function(position, player) {
  ...
  repeat {
    input <- readline(sprintf(
        "Player %s to move. What is your move?\n(\"A\", \"B\", \"C\"; or \"Q\" to quit): ",
        player
    ))
    ...
    valid <- tryCatch({
      return(ex02MoveStringToCoordinate(input, position))
...
ex04RandomPlayer <- function(position, player) {
  ...
  assertChoice(player, c("X", "O"))
  moves <- LETTERS[1:3]
  legalmoves <- moves[is.na(position[1, ])]
  ex02MoveStringToCoordinate(sample(legalmoves, 1), position)
}
...
ex05Tournament <- function(playerX, playerO, starting.player = "X", starting.position = matrix(NA_character_, 3, 3)) {
  ...
  repeat {
    ...
    coord <- players[[cp]](position, cp)
    assertIntegerish(coord, tol = 1e-100, lower = 1, upper = 3, len = 2, any.missing = FALSE)
    assertTRUE(all.equal(ex02MoveStringToCoordinate(LETTERS[[coord[[2]]]], position), coord))
    position[coord[[1]], coord[[2]]] <- cp
    message(sprintf("Player %s made move %s\nposition now:\n%s",
      cp, LETTERS[coord[[2]]], paste(capture.output(print(position)), collapse = "\n")))
